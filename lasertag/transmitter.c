...
#define TRANSMIT_HIGH_STR '1'
#define TRANSMIT_LOW_STR '0'
#define TRANSMIT_NEWLINE_STR '\n'
...
static uint16_t freqNum, transmitTimer, transmitCounter, tickTime,
    transmitCounterLength;
...
void transmitter_setFrequencyNumber(uint16_t frequencyNumber) {
  ...
  if (isContinuous || !transmitter_running()) {
    ...
    tickTime = filter_frequencyTickTable[frequencyNumber] / HALF_PERIOD;
  }
}
...
void transmitter_tick() {
  ...
  case transmit_high_st:
    ...
    if (!isContinuous && transmitCounter >= transmitCounterLength) {
      ...
      transmitter_set_jf1(TRANSMITTER_LOW_VALUE);
      DPCHAR(TRANSMIT_NEWLINE_STR);
    }
    break;
  case transmit_low_st:
    DPCHAR(TRANSMIT_LOW_STR);
    // Reached 200ms
    if (!isContinuous && transmitCounter >= transmitCounterLength) {
      currentState = waiting_st;
      transmitter_set_jf1(TRANSMITTER_LOW_VALUE);
...
void transmitter_runContinuousTest() {
  transmitter_setContinuousMode(true); // sets continuous flag to false
  transmitter_run();
